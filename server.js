...
app.delete('/api/people/me',(req,res)=>{
  ...
  if(!me)return res.status(401).json({error:'Not signed into your bloop account.'});
...
app.get('/api/people/discover',(req,res)=>{
  ...
  let rows;
  if(q.length>=2){
    rows=db.prepare(`SELECT id,username,display_name AS displayName,avatar_url AS avatarUrl,owner_token_hash AS ownerHash
  ...
  const people=rows.map(p=>({
    id:p.id,username:p.username,displayName:p.displayName,avatarUrl:p.avatarUrl||null,
